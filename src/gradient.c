...
int		get_gradient(t_gradient grad, t_point current, char axe)
{
	...
	green = get_gradient_mix((grad.start.color >> 8) & 0xFF, (grad.finish.color >> 8) & 0xFF, percent);
	blue = get_gradient_mix(grad.start.color & 0xFF, grad.finish.color & 0xFF, percent);
